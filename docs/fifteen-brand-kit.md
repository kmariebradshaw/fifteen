# Fifteen brand kit

The palette comes from the five benefit icons. Shared tokens and page treatments live in `assets/fifteen-brand.css`, loaded by `layout/theme.liquid`.

| Color | Hex | Role |
|---|---|---|
| Pink | #e83f74 | Primary calls to action, founder story, target |
| Blue | #1597bd | Clarity, convenience, graphic accents |
| Green | #48a26b | Everyday use, portability |
| Gold | #e4ac25 | Delight, small highlights |
| Purple | #78469c | Community and secondary accents |
| Navy | #071f48 | Body text and dark panels |
| Beige | #fffdf9 | Warm page base |
| Sky | #92d7ef | Blue gradient start |

## Usage
- Keep navy body text on light card fills. Gold is for icons, borders, and accents rather than small text on white.
- Pair each colored card border with its matching tint.
- Preserve the blue-to-white gradient: #92d7ef → #d9f1fa → #ffffff.
- Use the existing heading and body fonts, curved underlines, rounded cards, and pill buttons.
- Keep motion restrained and respect reduced motion.
- The shared stylesheet scopes treatments to Fifteen sections. Existing section color settings still drive base text and primary buttons; shared treatments control the additional brand accents.
